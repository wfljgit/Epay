...
$channels = $DB->getAll("SELECT id,name,plugin FROM pre_channel WHERE plugin IN ('".implode("','",$support_plugins)."') ORDER BY id ASC");
$channel_select = '';
				...
				<form class="form-horizontal" id="form-store">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							...
							<font color="green">只支持adapay支付插件</font>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input type="text" class="form-control" name="card_id" id="card_id" placeholder="">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right">银行卡姓名</label>
						<div class="col-sm-9">
							<input type="text" class="form-control" name="card_name" id="card_name" placeholder="">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right">身份证号码</label>
						<div class="col-sm-9">
							<input type="text" class="form-control" name="cert_id" id="cert_id" placeholder="会对银行卡号、银行卡姓名、身份证号三要素认证">
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<font color="green">一般限制最高30%</font>
						</div>
					</div>
					<div class="form-group">
...
<form action="./ps_receiver2.php" method="GET" class="form-inline">
  <div class="form-group">
    <label><b>搜索</b></label>
	...
	<a href="?" class="btn btn-default" title="刷新列表"><i class="fa fa-refresh"></i></a>
      ...
      <div class="table-responsive">
        <table class="table table-striped">
          ...
          <tbody>
<?php
$numrows=$DB->getColumn("SELECT count(*) from pre_psreceiver2 A WHERE{$sql}");
$pagesize=15;
$pages=ceil($numrows/$pagesize);
$page=isset($_GET['page'])?intval($_GET['page']):1;
$offset=$pagesize*($page - 1);

$rs=$DB->query("SELECT A.*,B.name channelname FROM pre_psreceiver2 A LEFT JOIN pre_channel B ON A.channel=B.id WHERE{$sql} order by A.id desc limit $offset,$pagesize");
while($res = $rs->fetch())
{
echo '<tr><td><b>'.$res['id'].'</b></td><td>'.$res['channel'].'__'.$res['channelname'].'</td><td><a href="./ulist.php?my=search&column=uid&value='.$res['uid'].'" target="_blank">'.$res['uid'].'</a></td><td>'.$res['card_id'].'／'.$res['card_name'].'</td><td>'.$res['rate'].'</td><td>'.($res['status']==1?'<a class="btn btn-xs btn-success" onclick="setStatus('.$res['id'].',0)">已开启</a>':'<a class="btn btn-xs btn-warning" onclick="setStatus('.$res['id'].',1)">已关闭</a>').'</td><td><a class="btn btn-xs btn-info" onclick="editframe('.$res['id'].')">编辑</a>&nbsp;<a class="btn btn-xs btn-danger" onclick="delItem('.$res['id'].')">删除</a>&nbsp;<a href="./ps_order.php?rid='.$res['id'].'" target="_blank" class="btn btn-xs btn-default">订单</a></td></tr>';
}
?>
          </tbody>
        </table>
...
if ($page>1)
{
echo '<li><a href="ps_receiver2.php?page='.$first.$link.'">首页</a></li>';
echo '<li><a href="ps_receiver2.php?page='.$prev.$link.'">&laquo;</a></li>';
} else {
...
echo '<li><a href="ps_receiver2.php?page='.$i.$link.'">'.$i .'</a></li>';
echo '<li class="disabled"><a>'.$page.'</a></li>';
for ($i=$page+1;$i<=$end;$i++)
echo '<li><a href="ps_receiver2.php?page='.$i.$link.'">'.$i .'</a></li>';
if ($page<$pages)
{
echo '<li><a href="ps_receiver2.php?page='.$next.$link.'">&raquo;</a></li>';
echo '<li><a href="ps_receiver2.php?page='.$last.$link.'">尾页</a></li>';
} else {
...
function editframe(id){
	...
	$.ajax({
		type : 'GET',
		url : 'ajax_profitsharing.php?act=get_receiver2&id='+id,
		dataType : 'json',
		success : function(data) {
			...
			if(data.code == 0){
				...
				$("#card_id").val(data.data.card_id);
				$("#card_name").val(data.data.card_name);
				$("#cert_id").val(data.data.cert_id);
				$("#tel_no").val(data.data.tel_no);
...
function save(){
	...
	var url = 'ajax_profitsharing.php?act=add_receiver2';
	if($("#action").val() == 'edit') url = 'ajax_profitsharing.php?act=edit_receiver2';
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	$.ajax({
		type : 'POST',
		url : url,
...
function delItem(id) {
	var confirmobj = layer.confirm('你确实要删除此分账规则吗？', {
	  ...
	  $.ajax({
		type : 'POST',
		url : 'ajax_profitsharing.php?act=del_receiver2',
		data : {id: id},
		dataType : 'json',
		success : function(data) {
			if(data.code == 0){
...
function setStatus(id,status) {
	...
	$.ajax({
		type : 'POST',
		url : 'ajax_profitsharing.php?act=set_receiver2',
